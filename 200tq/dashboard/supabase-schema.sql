...
-- 1) TABLES
-- =========================
-- [Phase 5C] Trade Execution Records (개인용, 1일 1기록)
create table if not exists trade_executions (
  ...
  -- 판정일
  snapshot_verdict text not null,
  -- 'ON' | 'OFF10'
...
create index if not exists trade_exec_date_idx on trade_executions(execution_date desc);
-- Holdings Snapshots
create table if not exists holdings_snapshots (
  ...
  -- { "TQQQ": {"shares": 100, "price": 50.0, "value": 5000}, ... }
  alloc_json jsonb,
  -- { "TQQQ": 0.62, "SPLG": 0.18, ... }
...
create table if not exists execution_checklists (
  ...
  created_at timestamptz default now(),
  as_of_date date not null,
  pretrade_json jsonb not null,
  -- { "fx_rate": 1400, "plan": {...}, "broker_checked": true }
  pretrade_complete boolean default false,
  posttrade_json jsonb not null,
  -- { "executed": {...}, "fees": 50, ... }
  executed_complete boolean default false,
  override_used boolean default false,
  -- If true, ignore complete flags in gate
  user_id uuid default auth.uid()
);
...
create table if not exists decision_attachments (
  ...
  as_of_date date not null,
  storage_path text not null,
  -- "user_id/as_of_date/filename.png"
  file_name text,
  mime_type text,
  user_id uuid default auth.uid()
);
...
if not exists (
  select 1
  from pg_constraint
  where conname = 'signals_regime_check'
) then
alter table signals
add constraint signals_regime_check check (regime in ('ON', 'OFF'));
end if;
-- [NEW] Daily Unique Constraints per User
if not exists (
  select 1
  from pg_constraint
  where conname = 'uniq_trigger_user_date'
) then
alter table trigger_evaluations
add constraint uniq_trigger_user_date unique (user_id, as_of_date);
end if;
if not exists (
  select 1
  from pg_constraint
  where conname = 'uniq_checklist_user_date'
) then
alter table execution_checklists
add constraint uniq_checklist_user_date unique (user_id, as_of_date);
end if;
if not exists (
  select 1
  from pg_constraint
  where conname = 'uniq_decision_user_date'
) then
alter table decision_logs
add constraint uniq_decision_user_date unique (user_id, as_of_date);
end if;
end $$;
-- =========================
-- 3) INDEXES
...
do $$
declare t text;
begin foreach t in array array ['holdings_snapshots', 'signals', 'news_items', 'reports', 'audit_log', 'trigger_evaluations', 'execution_checklists', 'decision_logs', 'decision_attachments'] loop -- Drop old
execute format(
...
-- Create new
execute format(
  'create policy "%1$s_select_own" on %1$s for select using (user_id = auth.uid())',
  t
);
execute format(
  'create policy "%1$s_insert_own" on %1$s for insert with check (user_id = auth.uid())',
  t
);
execute format(
  'create policy "%1$s_update_own" on %1$s for update using (user_id = auth.uid()) with check (user_id = auth.uid())',
  t
);
execute format(
  'create policy "%1$s_delete_own" on %1$s for delete using (user_id = auth.uid())',
  t
);
end loop;
end $$;